...
const AddressModal = () => {
  ...
  return (
    <>
      ...
      <>
      <div
        className={`fixed bottom-0 left-0 w-full transition-all duration-300 ease-in-out bg-white z-10 rounded-t-3xl overflow-hidden ${height ? 'h-full' : 'h-[50%]'
          }`}
      >
        {/* 모달창 크기 컨트롤 버튼*/}
        <div
        ...
        <div className='m-[20px] relative h-[50px]'>
          {(isFocused || query) &&<MdArrowBackIosNew style={arrowStyle} onClick={() => {
            setQuery('')
            setResults([])
          }}/>}
        ...
        <form onSubmit={searchAddress} className="flex-1 px-[20px] relative">
          ...
          <input
            ...
            onFocus={() => {setIsFocused(true), setHeight(true)}}
