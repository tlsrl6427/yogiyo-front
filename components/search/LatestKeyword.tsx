    ...
    <>{keywords.length > 0 &&
      <div className="px-[20px]">
        <div className="flex justify-around items-center">
          <h2 className="gap-2 w-full h-[50px] flex justify-start items-center bg-white text-[1.25rem] font-bold">최근 검색어</h2>
          <span className="text-[0.8rem] whitespace-nowrap text-slate-400 cursor-pointer" onClick={allDelete}>전체삭제</span>
        </div>
        <div className="w-full overflow-hidden overflow-x-auto no-scroll">
          <div className="flex gap-2">
            {keywords.map((keyword, i) => (
              <div key={i} className="flex gap-1 px-[8px] py-[6px] border rounded-3xl border-slate-300">
                <Link href={`/search/searchResult/?searchKeyword=${keyword}`} className="cursor-pointer whitespace-nowrap">{keyword}</Link>
                <i className="text-[2rem] relative cursor-pointer w-[20px] h-[20px]" onClick={() => keywordDelete(i)}>
